...
using AspireOllama.A2A.ResearchAgent.Tools;
...
var ollamaUrl = builder.Configuration["ConnectionStrings:ollama-research"] ?? "http://localhost:11434";
...
ResearchTools.Initialize(ollamaClient);
...
// MCP endpoints
app.MapMcp("/mcp");
app.MapGet("/", () => "Research Agent v2.0 (AI-Powered + A2A) is running. Endpoints: /mcp (MCP), /.well-known/agent.json (A2A)");
app.MapGet("/health", () => Results.Ok(new { status = "healthy", agent = "research", version = "2.0", ai = true, a2a = true }));
